//
//  Content.swift
//  Viafoura
//
//  Created by Martin De Simone on 19/07/2023.
//

import Foundation
public struct Content {
    let type: ContentType
    let story: Story?
    let poll: Poll?
    let liveChat: LiveChat?
    let liveQuestions: LiveQuestions?

    init(type: ContentType, story: Story? = nil, poll: Poll? = nil, liveChat: LiveChat? = nil, liveQuestions: LiveQuestions? = nil) {
        self.type = type
        self.story = story
        self.poll = poll
        self.liveChat = liveChat
        self.liveQuestions = liveQuestions
    }
}

public enum ContentType {
    case story
    case poll
    case liveChat
    case liveQuestions
}

enum LiveChatType {
    case portraitOverlay
    case fullscreen
    case portrait
}

struct LiveChat {
    let title: String
    let image: String
    let containerId: String
    let type: LiveChatType
}

struct LiveQuestions {
    let title: String
    let containerId: String
    var focusedContentUUID: UUID? = nil
}

public let defaultContents: [Content] = [
    Content(type: .liveQuestions, story: nil, poll: nil, liveChat: nil, liveQuestions: LiveQuestions(title: "Live Q&A", containerId: "600042387794")),
    Content(type: .story, story: Story(pictureUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTFbzIdlV4MXH5nPdmfcbCGSpB2ftxPNtQ10d9G8UUJCVtEgtYLKsLBAMDg&s=10", title: "Moving Staff to Cover the Coronavirus", description: "Here Are What Media Companies Are Doing to Deal With COVID-19 Information Overload", author: "Norman Phillips", category: "ECONOMY", link: "https://viafoura-mobile-demo.vercel.app/posts/here-are-what-media-companies-are-doing-with-covid-19-overload", containerId: "1994-7e39-5de6-8a3c-9a643ddc103d", authorId: "3147700024522", storyType: .comments), poll: nil),
    Content(type: .liveChat, story: nil, poll: nil, liveChat: LiveChat(title: "Sports live chat", image: "message.fill", containerId: "sports", type: .portraitOverlay), liveQuestions: nil),
    Content(type: .poll, story: nil, poll: Poll(contentContainerUUID: UUID(uuidString: "9BC06A89-2B04-402F-8379-D4E21A249B3C")!, title: "Who is the best player ever?")),
    Content(type: .story, story: Story(pictureUrl: "https://www.datocms-assets.com/55856/1636663477-blognewheights.jpg?fit=crop&fm=webp&h=428&w=856", title: "Grow civility", description: "Don't shut out your community, instead guide them towards civility", author: "Tom Hardington", category: "ECONOMY", link: "https://viafoura-mobile-demo.vercel.app/posts/dont-shut-out-your-community-guide-them-to-civility", containerId: "231414", authorId: "3147700024522", storyType: .comments), poll: nil),
    Content(type: .story, story: Story(pictureUrl: "https://www.datocms-assets.com/67251/1701970811-tacos.jpg?fit=crop&fm=webp&h=428&w=856", title: "Korean Fusion Delight", description: "Homemade Bulgogi Tacos Recipe", author: "Tom Hardington", category: "COOKING", link: "https://viafoura-mobile-demo.vercel.app/posts/brexit-to-cost-gbp1-200-for-each-person-in-uk", containerId: "a0335064233e55d-4442-aa6b-7fdcfe54636b", authorId: "3147700024522", storyType: .reviews), poll: nil),
    Content(type: .liveChat, story: nil, poll: nil, liveChat: LiveChat(title: "Business live chat", image: "bubble.left.and.bubble.right.fill", containerId: "business", type: .fullscreen), liveQuestions: nil)
]
